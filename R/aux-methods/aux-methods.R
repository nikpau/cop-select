# Mean square error function set up to dynamically work with the Monte Carlo Functions. 
MSE <- function(dataframe,parStart,parEnd,stepsize) {

        #Initialize means vector
        means <- vector()

        #Remove Inf values if they occur due to boundary problems of the calculations
        dataframe <- dataframe[is.finite(rowSums(dataframe)),]

        for (i in 1:ncol(dataframe)) {

                #write the mean of every i-th column to the i-th entry of the means vector
                means[i] <- mean(dataframe[,i])

        }
        #calculate MSE and write results in a data frame
        mse <- data.frame(means = means, Param = seq(parStart,parEnd,stepsize),
                          mse = (seq(parStart,parEnd,stepsize) - means)^2)
        mse
}


#Set up Empirical version of Blomqvist's Beta
blomqvist.beta <- function(u1, u2) {
        ...
        #Define quadrants
        bl <- c(u1[u1 < median.x & u2 < median.y],u2[u1 < median.x & u2 < median.y]) #Bottom left
        tl <- c(u1[u1 < median.x & u2 > median.y],u2[u1 < median.x & u2 > median.y]) #Top left
        br <- c(u1[u1 > median.x & u2 < median.y],u2[u1 > median.x & u2 < median.y]) #Bottom right
        tr <- c(u1[u1 > median.x & u2 > median.y],u2[u1 > median.x & u2 > median.y]) #Top right

        #Count observations in diagonally opposing quadrants
        n1 <- length(bl) + length(tr)
        n2 <- length(tl) + length(br)

        #Calculate and return beta
        emp.beta <- (n1-n2)/(n1+n2)
        emp.beta

}


#Numerical inversion of the Parameter-to-beta function for the Frank and Clayton
#copula to receive a Parameter, given a beta value.
FrankBetaToPar <- function(beta) {
        ...
        #Function to be inverted by finding a unit root such that y - f(x) = 0
        FrankParToBeta <- function(par){

                (4/par)*log(cosh(par/4))
        }  
        #Finding the root
        v <- uniroot(function(x) beta - FrankParToBeta(x),
                     lower = 0 + .Machine$double.eps^0.5, upper = 100,
                     tol = .Machine$double.eps^0.5, extendInt = "yes")$root
        return(a*v) #Return Parameter and multiply with sign
}


#Same function but for the Clayton copula
ClaytonBetaToPar <- function(beta) {
        ...
        #Function to be inverted by finding a unit root such that y - f(x) = 0
        ClaytonParToBeta <- function(par){

                -1+4*(2^(par+1)-1)^(-1/par)

        }
        #Finding the root
        v <- uniroot(function(x) beta - ClaytonParToBeta(x),
                     lower = 0 + .Machine$double.eps^0.5, upper = 20,
                     tol = .Machine$double.eps^0.5, extendInt = "yes")$root
        return(a*v) #Return Parameter and multiply with sign
}


#Numerical inversion of the Par to Tau function for the Frank copula to receive a Parameter, given a Tau Value.
FrankTauToPar <- function(tau) {
        ...
        #Function to be inverted by finding a unit root such that y - f(x) = 0
        FrankParToTau <- function(par){

                (1-(4/par)+4*(debye_1(par)/par))

        }

        #Finding the root
        v <- uniroot(function(x) tau - FrankParToTau(x),
                     lower = 0 + .Machine$double.eps^0.5, upper = 100,
                     tol = .Machine$double.eps^0.5, extendInt = "yes")$root
        return(a*v) #Return Parameter and multiply with sign
}
...
copulaSelection <- function(u1,u2, benchmark = F){
        ...
        tau <- kendall.tau(u1,u2,T) #from the VGAM package
        ...
        lliks <- list(gaussian_llik <- function(rho){ #Gaussian
                             ...
                             gumbel_llik <- function(theta){
                                     ...
                                     -sum(-pow(e1,1.0/(theta))+(2.0/(theta)-2.0)*log(e1)+(theta-1.0)*log(log(u1)*log(u2))-log(u1*u2)+log(((theta-1.0)*pow(e1,-1.0/(theta))+1)))
                             },
        ...
        agg <- data.frame(EstMethod = rep("",3),
                          Family = rep("",3),
                          AIC = as.double(rep(0,3)), 
                          estPar = rep(0,3))

        #Only write the row with lowest AIC per estimation method
        ifelse(identical(which.min(agg_ml$AIC),numeric(0))==T,
               agg[1,] <- c(rep(0,4)),
               agg[1,] <- agg_ml[which.min(agg_ml$AIC),])

        ifelse(identical(which.min(agg_iTau$AIC),numeric(0))==T,
               agg[2,] <- c(rep(0,4)),
               agg[2,] <- agg_iTau[which.min(agg_ml$AIC),])

        ifelse(identical(which.min(agg_iBeta$AIC),numeric(0))==T,
               agg[3,] <- c(rep(0,4)),
               agg[3,] <- agg_iBeta[which.min(agg_ml$AIC),])
...
#MonteCarlo Simulation of the Benchmarking for the copula selection process
copulaSelectionBenchmark <- function(N,K,family,parStart,parEnd,stepsize){


        #Initialize interim result matrix
        cumResults <- data.frame(min = c(0,0,0),
                                 median = c(0,0,0),
                                 max = c(0,0,0))

        mcBenchmark <- function(samplesize,family,par){
                ...
                for (i in 1:K) {
                        ...
                        #write benchmark results to matrix
                        benchmark <- summary(copulaSelection(u1,u2,T)$Benchmark)

                        #Add up times it takes to select a copula based on the copulaSelecion function
                        cumResults <- cumResults + benchmark[,c(2,5,7)]
        ...
        #Final result vector
        totResults <- data.frame(min = c(0,0,0),
                                 median = c(0,0,0),
                                 max = c(0,0,0))


        #For every parameter, add up results
        for (i in seq(parStart,parEnd,stepsize)){

                totResults <- totResults + mcBenchmark(N,family,i)

        }
        #Divide by number of parameters to return an average
        totResults/length(seq(parStart,parEnd,stepsize))
